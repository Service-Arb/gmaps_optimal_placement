...
//!        │
//!        │   $XDG_DATA_HOME/service_arb/pins-<study>.json
...
use service_arb_core::{Candidate, Payload, model::Report};
...
fn path(study: &str) -> Result<PathBuf, ServerFnError> {
	xdg::BaseDirectories::with_prefix("service_arb")
...
mod imp {
	use leptos::prelude::*;
	use service_arb_core::Candidate;
	...
	fn diff(payload: &service_arb_core::Payload, core: &[Pin]) -> PinFile {
		PinFile {
