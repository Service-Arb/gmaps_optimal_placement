...
impl State {
		...
		self.heavy.with_value(|h| {
			...
			self.report.set(Some(Report {
				...
				rows: picked
					.iter()
					.enumerate()
					.map(|(k, r)| service_arb_core::model::Row {
	...
	fn compare(&self) {
		let cands: Vec<service_arb_core::Candidate> = self.core.get_untracked().into_iter().map(|p| service_arb_core::Candidate { name: p.name, at: p.at }).collect();
		if cands.is_empty() {
			return;
		}
		self.selected.set(None);
...
mod imp {
	use leptos::prelude::*;
	use service_arb_core::model::{LayerSpec, colorise};
	...
	async fn load(s: State) {
		let payload = match gloo_net::http::Request::get("/payload.json").send().await {
			...
			Ok(r) => match r.json::<service_arb_core::Payload>().await {
				Ok(p) => p,
				Err(e) => return s.banner.set(Some(format!("⚠ the study did not parse — {e}"))),
			},
		};
		let model = match service_arb_core::Model::try_new(payload) {
