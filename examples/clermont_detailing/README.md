...
The prototype this repo's general tool grows out of. Open
**`tmp/geo/out/map.html`** in a browser: Google Maps basemap, competitors on it,
INSEE 200 m socio-economic grid painted over the top.

Sources live here, bulk downloads and generated maps live in `tmp/geo/`
(gitignored) — override with `GEO_WORK=/some/dir`.
...
```fish
cd examples/clermont_detailing
curl -L -o ../../tmp/geo/data/filosofi2021_200m_csv.zip \
  https://www.insee.fr/fr/statistiques/fichier/8735162/Filosofi2021_carreaux_200m_csv.zip
nix-shell -p "python3.withPackages(ps: [ps.pyproj])" --run "python3 build_grid.py"
python3 fetch_competitors.py     # cached in tmp/geo/data/places_cache, reruns are free
...
```fish
cd ../../tmp/geo/out; python3 -m http.server 8731 &
cd -; node smoke.js          # drives headless Chromium over CDP
```
